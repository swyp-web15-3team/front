...
컴포넌트와 타입/인터페이스는 동일한 기준으로 분리한다: 특정 페이지 전용인지, 2곳 이상에서 공유하는지로 나눈다.

- 페이지 전용 컴포넌트: 별도 폴더 이동 없이 해당 라우트의 `app/(route)/_components/`에 둔다
- 2곳 이상에서 쓰이는 컴포넌트: `components/`에 둔다
- 단일 컴포넌트/페이지에서만 쓰는 타입: 별도 파일 없이 해당 컴포넌트/페이지 파일 상단에 선언한다
- 2곳 이상에서 공유하는 타입: `types/` 폴더에 분리한다. 도메인이 있으면 도메인별 파일로(`types/user.ts`), 도메인이 없으면 공통 파일로(`types/common.ts`) 나눈다
